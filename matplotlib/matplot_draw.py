...
class preprocess():
    ...
    def final_dataframe(self,restdatadict):
    ...
    # def add_column_to_dataframe(self,monthdict,restdatadict):
        ...
        for rest in restdatadict:
            ...
            restdatadict[rest]["season"] = monthlist##插入季節欄位、之後會map成season
            ...
            for brand in ["WangPin","ThaiTown"]:
                ...
                if brand == 'WangPin':
                    try:
                        ...
                        brandcount_by_month_list_wanping.append(0)
                else:
                    try:
                        brandcount_by_month_list_wachen.append((brandcount_by_month.loc[:,"Month"].value_counts()).values[0])
                    except:
                        brandcount_by_month_list_wachen.append(0)
        # print(brandcount_by_month_list_wanping)
        # print(brandcount_by_month_list_wachen)
            ...
            for brand in ["WangPin","ThaiTown"]:
                ...
                if brand == 'WangPin':
                    try:
                        ...
                        brandcount_by_season_list_wanping.append(0)
                else:
                    try:
                        brandcount_by_season_list_wachen.append(brandcount_by_season.loc[:,"Season"].value_counts().values[0])
                    except:
                        brandcount_by_season_list_wachen.append(0)
        # print(brandcount_by_season_list_wanping)
        # print(brandcount_by_season_list_wachen)
        ##brandcount_by_season_list (春王品、瓦城)、(夏王品、瓦城)、(秋王品、瓦城)、 (冬王品、瓦城)
        x_season = seasonname
        y1_season = brandcount_by_season_list_wanping
        y2_season = brandcount_by_season_list_wachen
